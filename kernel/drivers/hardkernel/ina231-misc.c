...
static void ina231_get_sensor_values(struct measurement_reading *my_measurement_reading)
{
    ...
    list_for_each(list_head, &SensorList)
    {
        ...
        if(index == 0)
        {
			struct ina231_sensor 	*sensor = gsensor->p;
			my_measurement_reading->a7_cur_uA  = sensor->cur_uA;
			my_measurement_reading->a7_cur_uV  = sensor->cur_uV;
			my_measurement_reading->a7_cur_uW  = sensor->cur_uW;
		}
		else if(index == 3)
		{
			struct ina231_sensor 	*sensor = gsensor->p;
			my_measurement_reading->a15_cur_uA = sensor->cur_uA;
			my_measurement_reading->a15_cur_uV = sensor->cur_uV;
			my_measurement_reading->a15_cur_uW = sensor->cur_uW;
		}
		index++;
    } 
}
EXPORT_SYMBOL(ina231_get_sensor_values);
